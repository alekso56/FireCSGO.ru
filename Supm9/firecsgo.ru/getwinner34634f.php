...
while($row = mysql_fetch_array($rs)) {
	if($row["userid"] == $winuser) {
		...
		mysql_query("INSERT INTO `messages` (`id`,`userid`,`msg`,`from`) VALUES ('','$winuser','Вы выиграли $jackpot1$, с шансом $wonpercent%','Победа')");
	} else {
		$tc = $row["userid"];
		mysql_query("INSERT INTO `messages` (`id`,`userid`,`msg`,`from`) VALUES ('','$tc','lost','SYSTEM')");
		mysql_query("INSERT INTO `messages` (`id`,`userid`,`msg`,`from`) VALUES ('','$tc','$winname выиграл $jackpot1$, с шансом $wonpercent%','Проигрыш')");
	}
}
